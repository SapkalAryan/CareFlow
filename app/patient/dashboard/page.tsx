...
import { FileText, ShoppingBag, Clock, RefreshCw, Calendar, ArrowRight, Pill } from 'lucide-react';
    ...
    <div className="flex min-h-[calc(100vh-4rem)] bg-slate-50">
      ...
      <div className="flex-1 p-6 md:p-8 max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">Patient Health Portal</h1>
                  ...
                  <div key={rx._id} className="py-3.5 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Medications: {rx.items?.map((i: any) => i.medicineName).join(', ')}
                      </p>
                    </div>
                    <Link href="/patient/orders">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-900">Recent Pharmacy Orders</h2>
              ...
              <div className="divide-y divide-slate-100">
                {orders.slice(0, 4).map((ord) => (
                  <div key={ord._id} className="py-3.5 flex items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-sm text-slate-900">{ord.orderId}</span>
                        <StatusBadge status={ord.status} />
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">
                        Pharmacy: {ord.pharmacyId?.pharmacyName || 'Assigned Pharmacy'}
                      </p>
                    </div>
